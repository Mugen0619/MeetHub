...
- CloudWatch Logsへのログ集約設定(AWS本番デプロイのIssueで対応する)
...
| spatie/laravel-health | 1.40系 | ALBヘルスチェック用エンドポイント |
...
本番環境では`LOG_LEVEL=info`とし、DEBUGを出力しない想定とする(AWS本番デプロイのIssueで設定する)。
...
- ログイン失敗・ロックアウトにはIPアドレス(`ip`)を記録する。ALB配下ではALBのIPになるため、AWS本番デプロイのIssueで`X-Forwarded-For`を信頼するプロキシ設定(TrustProxies)を行う必要がある
- 予期しない例外(ERROR)のメッセージには、DBのエラー内容等が含まれうる。ログはアプリケーション外部に公開しない前提とする

## ヘルスチェック

`spatie/laravel-health`を使い、以下の2つのエンドポイントを用意する(`routes/health.php`)。

| エンドポイント | 用途 | レスポンス |
|---|---|---|
| `GET /health` | ALBのヘルスチェック | 全チェック成功時は`200 {"healthy":true}`、1つでも失敗したら`503`。どのチェックが失敗したか等の詳細は返さない |
| `GET /health/details` | ローカル開発での確認用 | チェックごとの結果(JSON)。`.env`の`HEALTH_EXPOSE_DETAILS=true`の場合のみ有効で、未設定・`false`の場合は`404`を返す(本番では設定しない) |

- チェック項目(`AppServiceProvider`で登録): データベース接続(`DatabaseCheck`)、キャッシュの読み書き(`CacheCheck`。キャッシュはログイン試行回数の制限にも使うため)
- ALBから定期的に呼ばれるため、セッション(`SESSION_DRIVER=database`によるDBへの書き込み)を伴う`web`ミドルウェアグループには含めない
- チェック結果はリクエストのたびにその場で実行し、プロセス内のメモリ(`InMemoryHealthResultStore`)にだけ保持する。結果保存用のDBテーブルや、定期実行(スケジューラー)は使わない
- 正常なヘルスチェックはアクセスログに記録しない(ログが埋め尽くされるため)。失敗時(503)は記録する
- Laravel標準のヘルスチェック(`/up`)は、エンドポイントを1つにまとめるため廃止した

**留意点(AWS本番デプロイのIssueで再検討する)**: DB接続をチェックに含めているため、RDSの障害時は全タスクが`unhealthy`になる。ALBは全ターゲットが`unhealthy`の場合もリクエストを振り分け続ける(fail open)が、ECSのヘルスチェックにも同じエンドポイントを使うと、DB障害中にタスクの入れ替えが繰り返される可能性がある。ECS側のヘルスチェック設定・猶予期間とあわせて検討する。
