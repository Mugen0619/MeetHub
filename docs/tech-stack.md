...
| Laravel | 13.16系 | PHP 8.3〜8.5をサポート(13.3以降は実質8.4以上)。RAISETIMELINEのSpring Bootに相当するバックエンドフレームワーク |
| Livewire | 4.4系 | Blade + PHPのみで動的なUIを実装するためのフレームワーク。JavaScriptをほぼ書かずに画面のインタラクティブ部分(参加申込みボタン・いいねトグル・コメント投稿等)を実装する |
| Alpine.js | Livewireに同梱のバージョンに準拠(3.15系) | Livewireが内部で利用する軽量JSライブラリ。個別にバージョン管理・追加設定は行わない |
| Laravel Breeze(Livewireスタック) | Laravel 13系対応版 | ログイン・ユーザー登録・プロフィール編集画面の認証スキャフォールディングに使用 |
...
| Vite | 8.0系 | Laravel公式のVite統合(`laravel-vite-plugin`)経由でCSS/JSアセットをビルド |
| Tailwind CSS | 4.3系 | ユーティリティファーストCSS。Bladeテンプレート・Livewireコンポーネントのスタイリングに使用 |
...
| larastan/larastan(PHPStan) | 3系 | 静的解析。Eloquentモデル・リレーション等のLaravel固有の型推論に対応(Checkstyleに相当するLint) |
| Laravel Pint | 1.32系 | コードスタイルの自動整形 |
