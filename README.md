...
- アカウント: ユーザー登録・ログイン・ログアウト、プロフィールの表示・編集(表示名・自己紹介・アイコン・メールアドレス・パスワード)
...
| トップページ | プロフィール編集 |
|---|---|
| ![トップページ](./docs/images/top.png) | ![プロフィール編集](./docs/images/profile-edit.png) |
| 未ログインのユーザーに、MeetHubの紹介とログイン・新規登録への導線を表示する | 表示名・自己紹介・アイコン(画像1枚)・メールアドレスを変更できる |
...
| 定員に達したイベント。参加者一覧は主催者のみ閲覧できる | タイトル・開催日時・場所・説明・定員・画像を入力する | アイコン・自己紹介・フォロー数・主催イベント。フォロー / フォロー解除 |
...
| `tests/Browser/AccessibilityTest.php` | トップページ・ログイン・登録・イベント一覧・イベント詳細(参加者/主催者)・プロフィール・プロフィール編集画面をaxe-core(デフォルトルールセット、影響度minorまで全て)で検査する。違反が見つかった場合は、テストではなく画面(Blade・Livewireコンポーネント)を修正する |
