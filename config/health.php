...
return [
     ...
     */
    /*
     * MeetHub: ヘルスチェックはALBからのリクエストごとにその場で実行し(always_send_fresh_results)、
     * 結果は同じリクエスト内で読むだけなので、DBテーブルを必要としないInMemoryストアのみを使う。
     */
    'result_stores' => [
        /*
    ...
    /*
     * MeetHub独自の設定: チェックごとの詳細結果を返すエンドポイント(/health/details)を公開するか。
     * DB接続エラーのメッセージ等、内部情報を含むため、未設定時は非公開(404)とする。本番では設定しないこと。
     */
    'expose_details' => (bool) env('HEALTH_EXPOSE_DETAILS', false),
    ...
    'notifications' => [
         ...
         */
        // MeetHub: 外部への通知(メール・Slack)は対象外のため、既定では無効にする
